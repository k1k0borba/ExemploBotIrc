	...
	#self.SendCommand('JOIN ' + self.channel) #entra no canal
	    ...
	    #self.SendPong()
            ...
            if self.data.find ('hi CrazyBot31') !=-1:
                usernick = self.data.split("!")[0]
                self.SendCommand('PRIVMSG #hell'+ usernick+': Hi'+'\r\n' )
                self.ircMessage(usernick,'hi')
            if self.data.find('RedActor') !=-1:
                self.SendCommand('PRIVMSG #hell RedActor: hi Man')
            if self.data.find('.clear') !=-1:
                os.system("clear")
	    elif self.data.find('.cmd')	!=-1:
		if self.data.split("!")[0] ==':luc1f3r':
            ...
            print self.data #imprime tudo que vier do servidor
